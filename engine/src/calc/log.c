...
void log_add_entry(log_t *log, char *entry) {
    ...
    if (log->last_index) {
        ...
        new_offset = last_offset + (int)strlen(&log->mem[last_offset]) + 1;
        if (new_offset + strlen(entry) >= MEM_LEN) {
            new_offset = 0;
        }
    }
    int new_left = new_offset;
    int new_right = new_offset + (int)strlen(entry);
    for (long index = log->first_index; index <= log->last_index; index++) {
        int left = get_offset(log, index);
        int right = left + (int)strlen(&log->mem[left]);
